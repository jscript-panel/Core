...
public:
	STDMETHODIMP GetIDsOfNames(REFIID, OLECHAR** names, uint32_t, LCID, DISPID* dispids) final
	{
		...
		RETURN_IF_FAILED(init_type_info());

		const ULONG hash = LHashValOfName(LANG_NEUTRAL, names[0]);
		const auto it = s_cache.find(hash);
		...
		else
		{
			...
			s_cache.emplace(hash, dispids[0]);
		}
...
private:
	...
	inline static std::unordered_map<ULONG, DISPID> s_cache;
