...
void ScriptHost::ParsePreprocessor()
{
	...
	const auto start_pos = code.find_first(pre_start.data()) + pre_start.length();
	const auto end_pos = code.find_first(pre_end.data());
	std::string author, version;

	if (start_pos < end_pos && end_pos < code.length())
	{
		const auto pre_body = code.subString(start_pos, end_pos - start_pos);
